...
export const InfoAboutVZN = () => {
	const [visibleModalType, setVisibleModalType] =
    React.useState<TVisibleModal>('aboutVZN')
  const [isBottomSheet, setIsBottomSheet] = React.useState<boolean>(false)
  const headerProps: THeaderStyle = visibleModalType === 'aboutVZN' ? { style: ''} : { style: ['border', 'justifyStartH1'] }
  const { findVzn, listVzn, addBoOperation } = useStore(state => state)
  const [boOperationsState, setBoOperationsState] = React.useState<IBoOperations[]>([])
  ...
  const itemVzn = React.useMemo(() => listVzn.filter(
    (el) => {
      if (!code) return
      if (el.Code === code)
        return el
  ...
  function handleClickOperation(e: React.MouseEvent<HTMLUListElement>) {
	...
	function headerRenderProps(): React.ReactNode {
		return (
      <h1>
        {visibleModalType !== 'aboutVZN' ? <CloseIcon className="" onClick={() => setVisibleModalType("aboutVZN")} /> : null}
        {visibleModalType === 'aboutVZN' ? `ВЗН №${title} (${isConsignment ? 'Расход' : 'Приход'})` : visibleModalType === 'viewingComposition' ? `Элемент ВЗН №${title}` : 'Операция'}
        {visibleModalType === 'aboutVZN' ? <VerticalIcon onClick={()=> setIsBottomSheet(true)}/> : null}
			</h1>
		)
	}
	...
	return (
      ...
      {isBottomSheet ? <BottomSheet visible={setIsBottomSheet}>
